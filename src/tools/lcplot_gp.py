...
def plotlc_gp(time, meas, the, err, trange, outputname=None, autoadjust=True):
...
gpdatafile = glob.glob("./lc_*.gp_lcout") 
if len(gpdatafile) == 0:
  print("This script must be run in the directory containing the lc_RUNNAME.gp_lcout")
  print("    file produced with the 'lcout' command. No such file was not found here.")
  print("    Aborting")
  exit()
if len(gpdatafile) > 1:
  print("Warning: Multiple lc_RUNNAME.gp_lcout files found in this directory")
  print("    The default behavior is to plot the first one alphabetically")

gpdata = np.loadtxt(gpdatafile[0])
the += gpdata

####

plotlc_gp(time, meas, the, err, [50., 1570.], outputname='AllData')
plotlc_gp(time, meas, the, err, trange, outputname='zoom', autoadjust=True)
